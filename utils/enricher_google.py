...
def search_google_places(business_name: str, city: str, state: str) -> dict | None:
    ...
    params = urllib.parse.urlencode({
        ...
        "locationbias":   f"circle:50000@{city},{state},US",
        "key":            GOOGLE_API_KEY
    })

    try:
        url  = f"{PLACES_SEARCH_URL}?{params}"
...
def get_place_details(place_id: str) -> dict:
    ...
    params = urllib.parse.urlencode({
        "place_id": place_id,
        "fields":   "name,formatted_address,formatted_phone_number,website,url,address_components",
        "key":      GOOGLE_API_KEY
    })
...
def determine_qa_routing(match_score: float, found: bool) -> tuple[str, str]:
    """
    Returns (pre_qa_status, pre_qa_note) based on match score.
    pre_qa_status: 'auto_approve', 'review', 'flag'
    """
    if not found:
...
def enrich_with_google(business_id: int, business_name: str, city: str,
    ...
    google_address = details.get("formatted_address") or candidate.get("formatted_address", "")
...
def enrich_batch(market: str = "US-DE", limit: int = 20, dry_run: bool = False) -> dict:
    enriched = skipped = errors = auto_approved = flagged = 0
    ...
    for business_id, business_name, city, state, addr1, reg_addr in records:
        ...
        print(f"  {status_icon} {business_name[:40]:<40} | match={result['address_match_score']:.0%} | {result['pre_qa_status']} | {result.get('website_url') or 'no website'}")
    ...
    print(f"  {enriched} enriched | {auto_approved} auto-approve | {flagged} flagged | {skipped} skipped | {errors} errors")
    return {"enriched": enriched, "auto_approved": auto_approved, "flagged": flagged}
